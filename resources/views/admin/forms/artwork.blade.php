<form method="POST" action="/admin/media/artworks/{{ $artwork->id }}" class="">
    {!! csrf_field() !!}
    <div class="flex justify-between">
        <div class="w-1/2 px-6">
            <div class="my-6{{ $errors->has('title') ? ' has-error' : '' }}">
                ...
                @endif
                <input type="text" name="title" value="{{ old('title') ?? $artwork->getTranslation('title', 'en') }}" class="input-text">
            </div>
            <div class="my-6{{ $errors->has('description') ? ' has-error' : '' }}">
                ...
                @endif
                <textarea name="description" class="input-text h-64">{{ old('description') ?? $artwork->getTranslation('description', 'en') }}</textarea>
            </div>
        </div>
        <div class="w-1/2 px-6">
            <div class="my-6{{ $errors->has('zh_title') ? ' has-error' : '' }}">
                ...
                @endif
                <input type="text" name="zh_title" value="{{ old('zh_title') ?? $artwork->getTranslation('title', 'zh') }}" class="input-text">
            </div>
            ...
            <div class="my-6{{ $errors->has('zh_description') ? ' has-error' : '' }}">
                ...
                @endif
                <textarea name="zh_description" class="input-text h-64">{{ old('zh_description') ?? $artwork->getTranslation('description', 'zh') }}</textarea>
            </div>
        </div>
    </div>


    <div class="my-6 px-6 justify-end flex">
